...
import { CustomRequest } from '../middlewares/authenticateToken'; // Import CustomRequest from your middleware
...
// --- Create Product --
export const createProduct = async (req: CustomRequest, res: Response) => { // Changed Request to CustomRequest
  if (!req.currentUser) {
...
export const updateProduct = async (req: CustomRequest, res: Response) => { // Changed Request to CustomRequest
  if (!req.currentUser) {
    return res.status(401).json({ message: 'User not authenticated.' });
  }
...
export const deleteProduct = async (req: CustomRequest, res: Response) => { // Changed Request to CustomRequest
  if (!req.currentUser) {
    return res.status(401).json({ message: 'User not authenticated.' });
  }
